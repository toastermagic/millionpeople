(function () {
    ...
    var controllers = angular.module('peopleControllers', []);
    ...
    controllers.controller('HomeController', [
        ...
        'socket',
        'peopleService',
        function ($scope, $rootScope, $http, $mdDialog, $mdMedia, $mdGridLayout, $timeout, $filter, socket, peopleService) {
            ...
            socket.on('newPerson', function(newb) {
                addPerson(newb);
            });
            
            socket.on('newPeople', function(newbs) {
                angular.forEach(newbs, function(newb) {
                    addPerson(newb);
                });
            });

            socket.on('searchComplete', function(records) {
                console.info('search complete', records); 
            ...
            function clearUnkeptPeople() {
                for(var i = $scope.vm.people.length -1; i >= 0 ; i--){
                    if(!$scope.vm.people[i].keep){
                        $scope.vm.people.splice(i, 1);
                    }
                }
            }

            $scope.$watch('vm.searchTerm', function (term) {
                if ($scope.vm.searching) {
                    return;
                }

                $scope.vm.searching = true;
                
                angular.forEach($scope.vm.people, function(person) {
                    person.keep = term === '' || person.SearchKey.indexOf(term) !== -1;
                });

                socket.emit('streamRq', term);
            });

            function addPerson(person) {
                
                var exists = $filter('getById')($scope.vm.people, person.PersonId);
                ...
                if ($scope.vm.people.length < 50) {
                    person.id = person.PersonId;
                    person.color = randomColor();
                    person.col = person.Gender === 'Male' ? 1 : 2;
                    person.row = person.Gender === 'Female' ? 1 : 2;
                    person.avatar = 'avatar:svg-' + ((person.PersonId % 15) + 1).toString();
                    person.keep = true;
                    
                    $scope.vm.people.push(person);
                }
            }
